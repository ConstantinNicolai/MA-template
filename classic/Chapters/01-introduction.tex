...
% The intended scope for this thesis is collecting a dataset, validating the usability of said dataset, using it to perform predictions and lastly evaluating the quality of those predictions. \\
% In practice there are also limitations of the scope in terms of which hardware and which neural networks are included in the study. The hardware is limited to the Nvidia RTX 2080 TI and the Nvidia A30. The neural networks in use are all part of the Pytorch Torchvision library\footnote{\url{https://docs.pytorch.org/vision/0.22/}}. \\
% Lastly, due to the grid-search like nature of the study, there are points in the grid where incompatabilities occur. These can be a model which does not fit onto a GPU or a benchmark which takes a prohibitive amount of time for a specific setting. Given the amount of troubleshooting necessary to find workarounds for all possible edge cases, we take the freedom of excluding the ones of lesser practical applicability from the scope of this work.


% of little practical applicability, we take the freedom of not taking them into the scope of this work.
...
% the execution times, power readings and a number of related metrics are stored together with the
% Pytorch objects of the profiled operations.
%  The power measurements are conducted using nvidia-smi\footnote{\url{https://docs.nvidia.com/deploy/nvidia-smi/index.html}} that is running in the background while the benchmark is performed.\\
% , along with a sensible selection of
% related metrics.
...
% collected data.\\
% along with their respective measurement errors
...
% For several metrics of interest, our predictions provide insights helping to choose the most suitable GPU for a given neural network. 
